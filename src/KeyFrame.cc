...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, long unsigned int nAgentKFId):
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.mTrl), mnNumberOfOpt(0), mnAgentId(F.GetAgentId()), mnAgentKFId(nAgentKFId)
{
